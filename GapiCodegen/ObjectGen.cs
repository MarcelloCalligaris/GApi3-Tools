...
namespace GtkSharp.Generation {
	...
	public class ObjectGen : ObjectBase  {
		...
		private readonly List<string> customAttrs = new List<string> ();
		private readonly IList<XmlElement> strings = new List<XmlElement> ();
		private readonly IDictionary<string, ChildProperty> childProps = new Dictionary<string, ChildProperty> ();
		private static readonly IDictionary<string, DirectoryInfo> dirs = new Dictionary<string, DirectoryInfo> ();

		public ObjectGen (XmlElement ns, XmlElement elem) : base (ns, elem, false)
		{
			foreach (XmlNode node in elem.ChildNodes) {
				if(!(node is XmlElement member) ||
                   member.GetAttributeAsBoolean("hidden"))
					continue;
				...
				case "custom-attribute":
					customAttrs.Add (member.InnerXml);
					break;
		...
		private static string Studlify (string name)
        {
            ...
            {
                result.Append(char.ToUpper(sub[0]));
